...
export default function DashboardAppliedFiltersDialog(props: {
        ...
        {appliedFilters.map((filter, i) => {
          ...
          return (
            <li key={entity + i} id="applied-filter-dialog-li">
              <strong>Entity:</strong> {entity} | <strong>Field:</strong>{" "}
              {(field !== "" && <span>{field}</span>) || "Not specified"} |{" "}
              <strong>Value:</strong>{" "}
              {(value !== "" && <span>{value}</span>) || "Not specified"}
            </li>
          );
        })}
      </ul>
